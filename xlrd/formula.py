...
def get_externsheet_local_range(bk, refx, blah=0):
    try:
        ...
        print(
            "!!! get_externsheet_local_range: refx=%d, not in range(%d)" % (refx, len(bk._externsheet_info)),
            file=bk.logfile,
        )
        return (-101, -101)
...
def evaluate_name_formula(bk, nobj, namex, blah=0, level=0):
    ...
    if blah:
        print(
            "::: evaluate_name_formula %r %r %d %d %r level=%d" % (namex, nobj.name, fmlalen, bv, data, level),
            file=bk.logfile,
        )
    ...
    while 0 <= pos < fmlalen:
        ...
        if blah:
            print(
                "Pos:%d Op:0x%02x Name:t%s Sz:%d opcode:%02xh optype:%02xh" % (pos, op, oname, sz, opcode, optype),
                file=bk.logfile,
            )
        ...
        if opcode == 0x00:  # tArray
            ...
            if not func_attrs:
                ...
                if blah:
                    print("    FuncID=%d name=%s nargs=%d" % (funcx, func_name, nargs), file=bk.logfile)
            ...
            if not func_attrs:
                ...
                if blah:
                    print("    name: %r, min~max args: %d~%d" % (func_name, minargs, maxargs), file=bk.logfile)
            ...
            if blah:
                print(
                    "   origrefx=%d refx=%d tgtnamex=%d dodgy=%d" % (origrefx, refx, tgtnamex, dodgy), file=bk.logfile
                )
            if tgtnamex == namex:
                if blah:
...
def decompile_formula(bk, fmla, fmlalen, fmlatype=None, browx=None, bcolx=None, blah=0, level=0, r1c1=0):
    ...
    while 0 <= pos < fmlalen:
        ...
        if blah:
            print(
                "Pos:%d Op:0x%02x opname:t%s Sz:%d opcode:%02xh optype:%02xh" % (pos, op, oname, sz, opcode, optype),
                file=bk.logfile,
            )
            print("Stack =", stack, file=bk.logfile)
        if sz == -2:
    ...
    if blah:
        print(
            "End of formula. level=%d any_rel=%d any_err=%d stack=%r" % (level, not not any_rel, any_err, stack),
            file=bk.logfile,
        )
        if len(stack) >= 2:
            print("*** Stack has unprocessed args", file=bk.logfile)
...
def dump_formula(bk, data, fmlalen, bv, reldelta, blah=0, isname=0):
    ...
    while 0 <= pos < fmlalen:
        ...
        pos += sz
    if blah:
        print("End of formula. any_rel=%d any_err=%d stack=%r" % (not not any_rel, any_err, stack), file=bk.logfile)
        if len(stack) >= 2:
            print("*** Stack has unprocessed args", file=bk.logfile)
